...
public class CustomAuthenticationService extends AuthenticationService {
    ...
    public CustomAuthenticationService(String clientId) {
        this(clientId, null);
    }

    public CustomAuthenticationService(String clientId, String deviceCode) {
        super(EMPTY_URI);
    ...
    private McLoginResponse getLoginResponseFromToken(String accessToken) throws RequestException {
        ...
        if (response.XErr != 0) {
            if (response.XErr == 2148916233L) {
                throw new RequestException("Microsoft account does not have an Xbox Live account attached!");
            } else if (response.XErr == 2148916235L) {
                throw new RequestException("Xbox Live is not available in your country!");
            } else if (response.XErr == 2148916238L) {
                throw new RequestException("This account is a child account! Please add it to a family in order to log in.");
            } else {
                throw new RequestException("Error occurred while authenticating to Xbox Live! Error ID: " + response.XErr);
            }
        }
